...
export let selectDishes = "SELECT dish_id AS ID, dish_name AS Name, dish_description AS Description, Courses.course_name AS Course, Teams.team_name AS Team, Event_Years.year AS Year FROM Dishes JOIN Courses on Dishes.course_id = Courses.course_id JOIN Teams on Dishes.team_id = Teams.team_id JOIN Event_Years on Dishes.event_year_id = Event_Years.event_year_id ORDER BY Year, Team, Course ASC;";
...
export let insertDish = "INSERT INTO Dishes (dish_name, dish_description, course_id, team_id, event_year_id) VALUES (?, ?, ?, ?, ?);";
...
export let updateDish = 'UPDATE Dishes SET dish_name = ?, dish_description = ?, course_id = ?, team_id = ?, event_year_id = ? WHERE dish_id = ?;'
...
export let reloadDishes = "INSERT INTO Dishes (dish_name, dish_description, event_year_id, team_id, course_id) VALUES ('Crispy Calamari Rings', 'Lightly battered squid rings, deep-fried until golden, served with a tangy lemon aioli.', 1, 1, 1), ('Honey-Glazed Salmon Fillet', 'Fresh salmon glazed with a blend of honey, garlic, and soy, grilled to perfection, accompanied by asparagus spears.', 1, 2, 2), ('Triple Chocolate Mousse', 'Velvety layers of dark, milk, and white chocolate mousse, topped with a dusting of cocoa powder.', 1, 3, 3), ('Stuffed Portobello Caps', 'Meaty mushrooms filled with herbed goat cheese, spinach, and roasted red peppers, drizzled with balsamic glaze.', 1, 2, 1), ('Thyme-Infused Lamb Chops', 'Juicy lamb chops seasoned with fresh thyme and rosemary, seared to desired doneness, paired with roasted root vegetables.', 1, 3, 2), ('Roasted Beetroot Salad', 'Freshly roasted beetroots combined with feta cheese, walnuts, and a honey dressing.', 2, 1, 1), ('Mango Sorbet', 'A refreshing mango sorbet made from ripe mangoes and a hint of lime.', 2, 1, 3), ('Ginger Chicken Stir-Fry', 'Aromatic ginger-infused chicken stir-fried with crisp vegetables in a light soy sauce.', 2, 3, 2), ('Spinach and Feta Tart', 'A flaky tart filled with sautéed spinach and creamy feta cheese.', 2, 2, 1), ('Mocha Cheesecake', 'Creamy cheesecake with a hint of coffee and chocolate.', 2, 2, 3);"
